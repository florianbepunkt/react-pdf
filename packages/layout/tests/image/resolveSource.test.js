...
describe("image resolveSource", () => {
  describe("source", () => {
    ...
    it("resolves data buffer", () => {
      expect(resolveSource(SOURCE_DATA_BUFFER)).resolves.toBe(SOURCE_DATA_BUFFER);
    });
  ...
  describe("async", () => {
    ...
    it("resolves url object", () => {
      expect(resolveSource(Promise.resolve(SOURCE_URL_OBJECT))).resolves.toBe(SOURCE_URL_OBJECT);
    });

    it("resolves buffer", () => {
      expect(resolveSource(Promise.resolve(SOURCE_BUFFER))).resolves.toBe(SOURCE_BUFFER);
    });

    it("resolves data buffer", () => {
      expect(resolveSource(Promise.resolve(SOURCE_DATA_BUFFER))).resolves.toBe(SOURCE_DATA_BUFFER);
    });

    it("resolves blob", () => {
      expect(resolveSource(Promise.resolve(SOURCE_BLOB))).resolves.toBe(SOURCE_BLOB);
    });
  });
  ...
  describe("async factory", () => {
    ...
    it("resolves url object", () => {
      expect(resolveSource(async () => SOURCE_URL_OBJECT)).resolves.toBe(SOURCE_URL_OBJECT);
    });

    it("resolves buffer", () => {
      expect(resolveSource(async () => SOURCE_BUFFER)).resolves.toBe(SOURCE_BUFFER);
    });

    it("resolves data buffer", () => {
      expect(resolveSource(async () => SOURCE_DATA_BUFFER)).resolves.toBe(SOURCE_DATA_BUFFER);
    });

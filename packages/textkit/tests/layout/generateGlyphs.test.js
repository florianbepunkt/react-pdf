...
describe("generateGlyphs", () => {
  ...
  test("should return correctly generate simple string glyphs", () => {
    ...
    expect(pluck("xAdvance", result.runs[0].positions)).toEqual([8, 8, 8, 8, 8]);
  });
  ...
  test("should return correctly generate multi ligature glyphs", () => {
    ...
    expect(pluck("id", result.runs[0].glyphs)).toEqual([76, 111, 64257, 109, 64259, 100, 111]);
    expect(pluck("xAdvance", result.runs[0].positions)).toEqual([8, 8, 10, 8, 10, 8, 8]);
  });
